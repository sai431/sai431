...
import com.poc.springboot.Controller.EmployeeInfoController;
...
public class EmployeeInfoControllerTest {
	...
	@InjectMocks
	private EmployeeInfoController emp;
	...
	@Test
	public void createEmployeeInfoSuccessTest() {

		Employee employeeEntity = getEmployeeDto();

		EmpLoyeeInfoEntity empConv = getEmpInfoEntity();
		...
		Assert.assertEquals(empResponse.getEmployee().getSalbrkup().getBasic(), 2000, 0);
		Assert.assertEquals(empResponse.getEmployee().getSalbrkup().getCityCompAllow(), 2000, 0);
		Assert.assertEquals(empResponse.getEmployee().getSalbrkup().getEducation(), 2000, 0);
	}

	@Test
	public void updateEmpRespTestFailureCoverage() {
		Employee employeeEntity = new Employee();
		employeeEntity.setEmpId(1234L);
		employeeEntity.setEmpName("Rajesh");
		employeeEntity.setRole("Dev");
		employeeEntity.setSalaryCtc(25000L);
		when(breakUpRepo.existsById(1234l)).thenReturn(false);
	}

	@Test
	public void updateEmpRespTestSuccessPath() {
		Employee employeeEntity = getEmployeeDto();
		EmpLoyeeInfoEntity empConv = getEmpInfoEntity();
		SalaryBreakupEntity salaryEntity = getSalaryBreakupEntity();
		...
		when(empInfoRepo.save(Mockito.any())).thenReturn(empConv);
		when(breakUpRepo.save(Mockito.any())).thenReturn(salaryEntity);
		EmployeeRespDto empResponse = emp.updateEmpResp(1234L, 45823L, employeeEntity);
		Assert.assertNotNull(empResponse);
		Assert.assertEquals(empResponse.getEmployee().getEmpName(), "Rajesh");
		Assert.assertEquals(empResponse.getEmployee().getRole(), "Dev");
		Assert.assertEquals(empResponse.getEmployee().getSalbrkup().getBasic(), 2000, 0);
	...
	public void getEmployeeByNameTest() {
		...
		when(empInfoRepo.findByEmpName(Mockito.anyString())).thenReturn(getListOfEmployee());
		EmployeeRespDto empResponse = emp.getAllDetails("Rajesh");
		Assert.assertNotNull(empResponse);
		Assert.assertEquals(empResponse.getEmployee().getEmpName(), "Rajesh");
		Assert.assertEquals(empResponse.getEmployee().getRole(), "Dev");
		Assert.assertEquals(empResponse.getEmployee().getSalbrkup().getBasic(), 2000, 0);
	...
	@Test
	public void getallEmpSuccessCheck() {
		when(empInfoRepo.findAll()).thenReturn(getListOfEmployee());
		List<EmployeeRespDto> empResponse = emp.getallEmp();
		List<EmployeeRespDto> expectedsEmpResp = getListOfEmployeeRespDto();

		Assert.assertNotNull(empResponse);
		LOGGER.info(empResponse.toString());
		...
		Assert.assertEquals(empResponse.get(0).getEmployee().getSalbrkup().getEducation(),
	...
	private EmpLoyeeInfoEntity getEmpInfoEntity() {
		EmpLoyeeInfoEntity empConv = new EmpLoyeeInfoEntity();
		empConv.setEmpId(1234L);
		empConv.setEmpName("Rajesh");
		empConv.setRole("Dev");
		empConv.setSalaryCtc(25000L);
		return empConv;
	}

	private SalaryBreakupEntity getSalaryBreakupEntity() {
		SalaryBreakupEntity salaryEntity = new SalaryBreakupEntity();
		double dble = 2000;
		salaryEntity.setBasic(dble);
		salaryEntity.setCityCompAllow(dble);
		salaryEntity.setEducation(dble);
		return salaryEntity;
	}

	private Employee getEmployeeDto() {
		Employee employeeEntity = new Employee();
		employeeEntity.setEmpId(1234L);
		employeeEntity.setEmpName("Rajesh");
		employeeEntity.setRole("Dev");
	...
	private List<EmployeeRespDto> getListOfEmployeeRespDto() {
		...
		EmployeeRespDto empConv = new EmployeeRespDto();
		EmployeeRespDto empConv1 = new EmployeeRespDto();
		double dble = 2000;
		SalaryBreakUpModel salModel = new SalaryBreakUpModel();
		salModel.setBasic(dble);
		salModel.setEducation(dble);
		EmployeeConversionModel empconv = new EmployeeConversionModel();
		empconv.setEmpName("Rajesh");
		empconv.setRole("Dev");
		empconv.setSalbrkup(salModel);
		EmployeeConversionModel empconv1 = new EmployeeConversionModel();
		empconv1.setEmpName("Rajesh");
		empconv1.setRole("Dev");

		empconv1.setSalbrkup(salModel);

		empConv.setEmployee(empconv);
		empConv1.setEmployee(empconv1);

		listOfEmpl.add(empConv);
		listOfEmpl.add(empConv1);
	...
	private List<EmpLoyeeInfoEntity> getListOfEmployee() {
		...
		empConv.setRole("Dev");
		empConv.setSalaryCtc(25000L);
		SalaryBreakupEntity salbrkUpEntity = getSalaryBreakupEntity();

		empConv.setSalaryBreakupEntity(salbrkUpEntity);
		EmpLoyeeInfoEntity empConv1 = new EmpLoyeeInfoEntity();
		empConv1.setEmpId(1234L);
		empConv1.setEmpName("Rajesh");
		empConv1.setRole("Dev");
		empConv1.setSalaryCtc(25000L);
		listOfEmpl.add(empConv);
		listOfEmpl.add(empConv);
		return listOfEmpl;
